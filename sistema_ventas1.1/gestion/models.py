...
class Producto(models.Model):
    ...
    stock = models.IntegerField(default=0) # Stock en la tabla de Producto (Tabla 6)
...
class DevolucionAProveedorDetalle(models.Model):
    devolucion = models.ForeignKey(DevolucionAProveedor, on_delete=models.CASCADE)
    producto = models.ForeignKey('Producto', on_delete=models.PROTECT)
    cantidad = models.IntegerField()
...
# TABLA 42: Ajustes de Inventario (Cabecera) (corresponde a 'ajustes')
class Ajuste(models.Model):
    ...
    def __str__(self):
        return f'Ajuste #{self.id} ({self.fecha.strftime("%Y-%m-%d")})'

# TABLA 43: Detalle de Ajuste (corresponde a 'ajustes_detalle')
class AjusteDetalle(models.Model):
    ...
    def __str__(self):
        return f'{self.tipo_ajuste.nombre} de {self.producto.nombre} x{self.cantidad}'
    
# TABLA 44: Tabla Intermedia PedidoCliente/Venta (corresponde a 'pedido_cliente_has_venta')
...
def actualizar_stock_y_total_venta(sender, instance, created, **kwargs):
    """
    Señal que actualiza el stock y el total de la venta cada vez que se guarda un DetalleVenta.
    """
    ...
    if created:
        ...
        try:
            original = DetalleVenta.objects.get(pk=instance.pk)
    ...
    #  Rompe la recursión guardando solo el campo 'total'
...
def devolver_stock_y_total_venta(sender, instance, **kwargs):
    """
    Señal que devuelve stock y actualiza total al eliminar un DetalleVenta.
    """
    producto = instance.producto
    venta = instance.venta
...
def actualizar_stock_y_total_compra(sender, instance, created, **kwargs):
    """
    Señal que actualiza el stock y el total de la compra.
    """
    producto = instance.producto
    ...
    if created:
        ...
        try:
            original = DetalleCompra.objects.get(pk=instance.pk)
            diferencia = instance.cantidad - original.cantidad
...
def ajustar_stock_y_total_compra_eliminacion(sender, instance, **kwargs):
    """
    Señal que ajusta stock y total al eliminar un DetalleCompra.
    """
    producto = instance.producto
    compra = instance.compra
...
@receiver(post_save, sender=AjusteDetalle)
def actualizar_stock_por_ajuste(sender, instance, created, **kwargs):
    """
    Señal que ajusta stock de productos según tipo de ajuste.
    - Si es entrada: suma stock.
    - Si es salida: resta stock.
    """
    producto = instance.producto
    ajuste = instance.ajuste
    tipo_ajuste = instance.tipo_ajuste
