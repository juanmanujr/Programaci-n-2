...
class ProveedorDetailView(DetailView):
    ...
    model = Proveedor
    template_name = 'gestion/proveedores/proveedor_detalle.html' 
    context_object_name = 'proveedor'

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['titulo_pagina'] = f'Detalle de Proveedor: {self.object.nombre}'
        return context
...
class AjusteCreateView(BaseCreateView):
    ...
    # 💥 AÑADIMOS O REEMPLAZAMOS form_valid para asegurar el usuario
    def form_valid(self, form):
        # 1. Obtenemos el objeto Ajuste de la forma sin guardarlo aún
        ajuste = form.save(commit=False) 
        
        # 2. Asignamos la instancia del usuario logueado
        if self.request.user.is_authenticated:
            ajuste.usuario = self.request.user
        # Nota: Si el usuario no está autenticado, la vista con 
        # PermissionRequiredMixin ni siquiera debería ejecutarse.

        # 3. Guardamos el objeto Ajuste con el usuario correcto
        ajuste.save()
        
        # 4. Asignamos el objeto guardado a self.object para que get_success_url funcione
        self.object = ajuste
...
class AjusteDetalleView(PermissionRequiredMixin, UpdateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        form = AjusteInventarioForm(request.POST, instance=self.object)
        ...
        if form.is_valid() and formset.is_valid():
            try:
                with transaction.atomic():
                    ...
                    # 💥 CORRECCIÓN: Asignar el usuario antes de guardar el objeto
                    ajuste = form.save(commit=False)
                    
                    # 1. Asignar el usuario de la solicitud (requiere que el usuario esté logueado)
                    # Esto garantiza que se asigne una instancia de User, no una cadena.
                    if request.user.is_authenticated:
                        ajuste.usuario = request.user
                    else:
                        # Opcional: Si necesitas un valor de auditoría para usuarios no logueados,
                        # DEBES buscar una INSTANCIA de User, por ejemplo, un usuario 'sistema'.
                        # Si no quieres complicarte, simplemente asegura que siempre se acceda logueado.
                        # Aquí, vamos a asumir que el usuario siempre está logueado ya que usas PermissionRequiredMixin.
                        pass
                        
                    ajuste.save() # Guarda la cabecera después de asignar el usuario
                    
                    # 2. Guardar los detalles (formset)
                    formset.save()
                ...
                # El error que viste (Cannot assign...) se produce si no se asignó correctamente 'ajuste.usuario'
