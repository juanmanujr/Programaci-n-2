...
from django.db.models import Sum  # Importamos Sum para calcular totales
from .models import (
...
class AuditoriaAdmin(admin.ModelAdmin):
    ...
    # 🛑 RESTRICCIÓN CLAVE: NO permitir creación manual (Resuelve el IntegrityError)
    def has_add_permission(self, request):
        return False

    # Opcional: Impedir la edición/eliminación manual, ya que es un historial inmutable
...
class DevolucionAProveedorAdmin(admin.ModelAdmin):
    ...
    # FIX: Método que resuelve el SystemCheckError (admin.E108)
    def total_devuelto(self, obj):
        """Calcula y devuelve el monto total de la devolución."""
        # Asume la relación inversa 'devolucionaproveedordetalle_set' y un campo 'subtotal' en el detalle
        total = obj.devolucionaproveedordetalle_set.aggregate(
            total_sum=Sum('subtotal')
        )['total_sum']
        
        # Formato de moneda simple para la visualización
        return f"{total:,.2f}" if total is not None else "0.00"
    
    total_devuelto.short_description = 'Total Devuelto' # Nombre de la columna en el Admin
